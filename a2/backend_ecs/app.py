# Use the same logic as backend_ec2/app.py
...
def query_music():
    title, artist, year, album = request.args.get('title'), request.args.get('artist'), request.args.get('year'), request.args.get('album')
    if not any([title, artist, year, album]): return jsonify({"message": "At least one field must be completed."}), 400
    ...
    if artist:
        ...
        f = None
        if title: f = Attr('title').eq(title)
        if artist: f = f & Attr('artist').eq(artist) if f else Attr('artist').eq(artist)
        if album: f = f & Attr('album').eq(album) if f else Attr('album').eq(album)
        items = table.scan(FilterExpression=f).get('Items', [])
    if not items: return jsonify({"message": "No result is retrieved. Please query again"}), 404
    for item in items:
